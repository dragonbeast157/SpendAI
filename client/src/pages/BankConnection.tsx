...
export function BankConnection() {
  ...
  const handleUploadStatement = async (file: File) => {
    try {
      ...
      // For now, we'll use the first connected account or create a default one
      // In a real implementation, you might want to let the user select which account
      let targetAccountId = null;
      if (accounts.length > 0) {
        targetAccountId = accounts[0]._id;
        console.log("BankConnection: Using existing account:", targetAccountId);
      } else {
        console.log("BankConnection: No accounts found, using general upload");
        // Fall back to general upload if no accounts
      ...
      const response = await uploadBankStatementForAccount(targetAccountId, file);
