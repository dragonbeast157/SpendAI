...
export const uploadPolicyDocument = async (file: File, title?: string, description?: string) => {
  console.log('=== FRONTEND POLICY UPLOAD START ===');
  ...
  try {
    console.log('Frontend: Making actual API call to upload policy document');
    ...
    console.log('Frontend: Policy upload API response received');
    console.log('Frontend: Response status:', response.status);
    console.log('Frontend: Response data:', response.data);
    console.log('=== FRONTEND POLICY UPLOAD END ===');
    return response.data;
  } catch (error: any) {
    console.error('=== FRONTEND POLICY UPLOAD ERROR ===');
    console.error('Frontend: Policy upload error:', error);
    console.error('Frontend: Error response:', error?.response?.data);
    console.error('Frontend: Error status:', error?.response?.status);
    console.error('=== END FRONTEND POLICY UPLOAD ERROR ===');
    throw new Error(error?.response?.data?.message || error.message);
  }
};
...
export const createPolicy = async (data: {
  ...
  try {
    ...
    console.error('API createPolicy: Error status:', error?.response?.status);
    console.error('API createPolicy: Request URL:', error?.config?.url);
    throw new Error(error?.response?.data?.message || error.message);
  }
};
...
export const deletePolicy = async (id: string) => {
  ...
  try {
    ...
    console.error('API deletePolicy: Error status:', error?.response?.status);
    throw new Error(error?.response?.data?.message || error.message);
  }
};