  ...
  static async exportUserData(userId, format = 'json') {
    try {
      ...
      try {
        ...
        // Try different possible method names that might exist in BankAccountService
        let accountResult;
        if (typeof BankAccountService.getAccounts === 'function') {
          accountResult = await BankAccountService.getAccounts(userId);
        } else if (typeof BankAccountService.getUserAccounts === 'function') {
          accountResult = await BankAccountService.getUserAccounts(userId);
        } else if (typeof BankAccountService.findByUserId === 'function') {
          accountResult = await BankAccountService.findByUserId(userId);
        } else {
          console.log('SettingsService: No suitable method found in BankAccountService, available methods:', Object.getOwnPropertyNames(BankAccountService));
          throw new Error('BankAccountService method not found');
        }
        bankAccounts = accountResult.accounts || accountResult || [];
