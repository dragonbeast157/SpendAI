...
class TransactionService {
  ...
  static async create(userId, transactionData, userAccountType) {
    ...
    try {
      ...
      // Check for anomalies (basic implementation)
  ...
  static async detectAnomaly(userId, transactionData) {
    ...
    try {
      // Get user's transaction history for the same category
      ...
      console.log('TransactionService: Found', historicalTransactions.length, 'historical transactions for category:', transactionData.category);
      ...
      const moderateThreshold = avgAmount + (2 * stdDev); // 2 standard deviations
      const majorThreshold = avgAmount + (3 * stdDev); // 3 standard deviations
      const simpleThreshold = avgAmount * 2.5; // 2.5x the average as fallback
      ...
      if (currentAmount > majorThreshold || currentAmount > avgAmount * 4) {
        hasAnomaly = true;
        severity = 'major';
        ...
        console.log('TransactionService: MAJOR anomaly detected - currentAmount:', currentAmount, 'majorThreshold:', majorThreshold, 'avgAmount*4:', avgAmount * 4);
      } else if (currentAmount > moderateThreshold || currentAmount > simpleThreshold) {
        hasAnomaly = true;
        severity = 'moderate';
        reason = `This ${transactionData.category} expense is significantly higher than usual`;
        console.log('TransactionService: MODERATE anomaly detected - currentAmount:', currentAmount, 'moderateThreshold:', moderateThreshold, 'simpleThreshold:', simpleThreshold);
      } else if (currentAmount > avgAmount * 1.8) {
        hasAnomaly = true;
        severity = 'minor';
        reason = `This ${transactionData.category} expense is above your normal range`;
        console.log('TransactionService: MINOR anomaly detected - currentAmount:', currentAmount, 'avgAmount*1.8:', avgAmount * 1.8);
      } else {
